...
export async function createConversation(conversationData) {
  const client = await pool.connect();
  try {
    // FIXED: Add required fields that exist in the actual database table
    const query = `
      INSERT INTO conversations (customer_id, conversation_key, source, status)
      VALUES ($1, $2, $3, $4)
      RETURNING *
    `;
    ...
    const values = [
      ...
      conversationData.source || 'web',  // Default source
      conversationData.status || 'active' // Default status
    ];
...
export async function createMessage(messageData) {
  ...
  try {
    ...
    const result = await client.query(query, values);
    console.log('✅ Message saved:', messageData.sender);
    return result.rows[0];
  } catch (error) {
