  ...
  try {
    ...
    await client.query(`
      CREATE TABLE IF NOT EXISTS email_settings (
        id SERIAL PRIMARY KEY,
        customer_id INTEGER REFERENCES customers(id),
        setup_method VARCHAR(50) DEFAULT 'intellihub',
        ...
        ask_qualifying_questions BOOLEAN DEFAULT true,
...
export async function createEmailConversation(conversationData) {
  const client = await pool.connect();
  try {
    const query = `
      INSERT INTO email_conversations (customer_id, customer_email, business_email, subject, thread_id, status)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `;
    ...
    const values = [
      conversationData.customer_id,
      conversationData.customer_email,
      conversationData.business_email,
      conversationData.subject || 'New Email Conversation',
      conversationData.thread_id || null,
      conversationData.status || 'active'
    ];
    
    const result = await client.query(query, values);
    console.log('📧 Email conversation created:', result.rows[0].id);
    return result.rows[0];
  } catch (error) {
...
export async function getEmailConversationByAddress(customerEmail, customerId) {
  const client = await pool.connect();
  try {
    const query = 'SELECT * FROM email_conversations WHERE customer_email = $1 AND customer_id = $2 ORDER BY created_at DESC LIMIT 1';
    const result = await client.query(query, [customerEmail, customerId]);
    return result.rows[0] || null;
  } catch (error) {
    console.error('❌ Error getting email conversation by address:', error);
    return null;
  } finally {
    client.release();
  }
...
export async function getEmailConversationByThreadId(threadId) {
  const client = await pool.connect();
  try {
    const query = 'SELECT * FROM email_conversations WHERE thread_id = $1';
    const result = await client.query(query, [threadId]);
    return result.rows[0] || null;
  } catch (error) {
    console.error('❌ Error getting email conversation by thread ID:', error);
    return null;
  } finally {
    client.release();
  }
...
export async function getEmailConversationsByCustomer(customerId) {
  const client = await pool.connect();
  try {
    const query = 'SELECT * FROM email_conversations WHERE customer_id = $1 ORDER BY created_at DESC';
    const result = await client.query(query, [customerId]);
    return result.rows;
  } catch (error) {
    console.error('❌ Error getting email conversations by customer:', error);
    return [];
  } finally {
    client.release();
  }
...
export async function getAllEmailConversations() {
  const client = await pool.connect();
  try {
    const query = 'SELECT * FROM email_conversations ORDER BY created_at DESC';
    const result = await client.query(query);
    return result.rows;
  } catch (error) {
    console.error('❌ Error getting all email conversations:', error);
    return [];
  } finally {
    client.release();
  }
...
export async function createEmailMessage(messageData) {
  const client = await pool.connect();
  try {
    const query = `
      INSERT INTO email_messages (conversation_id, sender, content, html_content, message_id, in_reply_to)
      VALUES ($1, $2, $3, $4, $5, $6)
      RETURNING *
    `;
    ...
    const values = [
      ...
      messageData.sender,
      messageData.content,
      messageData.html_content || null,
      messageData.message_id || null,
      messageData.in_reply_to || null
    ];
    
    const result = await client.query(query, values);
    console.log('📧 Email message created:', result.rows[0].id);
    return result.rows[0];
  } catch (error) {
...
export async function getEmailMessages(conversationId) {
  const client = await pool.connect();
  try {
    const query = 'SELECT * FROM email_messages WHERE conversation_id = $1 ORDER BY created_at ASC';
    const result = await client.query(query, [conversationId]);
    return result.rows;
  } catch (error) {
...
export async function getEmailMessagesByCustomer(customerId) {
  const client = await pool.connect();
  try {
    const query = `
    ...
    console.error('❌ Error getting email messages by customer:', error);
    return [];
  } finally {
    client.release();
  }
...
export async function getAllEmailMessages() {
  const client = await pool.connect();
  try {
    const query = 'SELECT * FROM email_messages ORDER BY created_at DESC';
    const result = await client.query(query);
    return result.rows;
  } catch (error) {
    console.error('❌ Error getting all email messages:', error);
    return [];
  } finally {
    client.release();
  }
...
export async function getCustomerStats(customerId) {
  const client = await pool.connect();
  try {
    const [conversations, messages, hotLeads, smsConversations, smsMessages, emailConversations, emailMessages] = await Promise.all([
      ...
      client.query(`
        ...
        WHERE sc.customer_id = $1
      `, [customerId]),
      client.query('SELECT COUNT(*) as count FROM email_conversations WHERE customer_id = $1', [customerId]),
      client.query(`
    ...
    const stats = {
      ...
      total_email_conversations: parseInt(emailConversations.rows[0].count),
      total_email_messages: parseInt(emailMessages.rows[0].count)
    };
...
export async function getEmailConversations() { return getAllEmailConversations(); }
...
export async function debugDatabase() {
  const client = await pool.connect();
  try {
    const [customers, conversations, messages, hotLeads, smsConversations, smsMessages, emailConversations, emailMessages, emailSettings, emailTemplates] = await Promise.all([
      ...
      client.query('SELECT COUNT(*) as count FROM email_conversations'),
      client.query('SELECT COUNT(*) as count FROM email_messages'),
      client.query('SELECT COUNT(*) as count FROM email_settings'),
      client.query('SELECT COUNT(*) as count FROM email_templates')
    ]);
    ...
    console.log('Email Conversations:', emailConversations.rows[0].count);
    console.log('Email Messages:', emailMessages.rows[0].count);
    console.log('Email Settings:', emailSettings.rows[0].count);
    console.log('Email Templates:', emailTemplates.rows[0].count);
    console.log('==============================');
    ...
    return {
      ...
      emailConversations: emailConversations.rows[0].count,
      emailMessages: emailMessages.rows[0].count,
      emailSettings: emailSettings.rows[0].count,
      emailTemplates: emailTemplates.rows[0].count
    };
  } catch (error) {
