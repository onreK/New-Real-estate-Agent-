...
// COMPLETE DATABASE MIGRATION FILE FOR MULTI-TENANT ARCHITECTURE
...
async function createContactsTable() {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS contacts (
      ...
      -- Index for duplicate detection within a customer's account
...
async function createContactEventsTable() {
  ...
  await query(`
    CREATE TABLE IF NOT EXISTS contact_events (
      ...
      INDEX idx_contact_events_customer (customer_id),
      INDEX idx_contact_events_contact (contact_id),
      INDEX idx_contact_events_type (event_type),
      INDEX idx_contact_events_created (created_at DESC)
    )
  `);
