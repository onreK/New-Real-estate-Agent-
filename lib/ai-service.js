...
function buildChannelSpecificPrompt(channel, customerConfig, channelData = {}, leadContext = null) {
  ...
  // Document link — sent once a lead is clearly qualified and ready to proceed
  const documentLink = customerConfig?.document_link?.trim() || '';
  const documentDescription = customerConfig?.document_description?.trim() || '';
  if (documentLink && documentDescription) {
    basePrompt += `\n\nDOCUMENT: Once a lead is clearly interested and ready to move forward, include this link for them to complete before the job begins: ${documentDescription} — ${documentLink}. Present it naturally as a next step, not in every message.`;
  }
