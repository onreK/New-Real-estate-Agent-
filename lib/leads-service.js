...
// COMPLETE VERSION WITH ONLY SQL SYNTAX FIXES - All 1014 lines of functionality preserved
// Fixed: Replaced FILTER syntax with CASE statements for PostgreSQL compatibility
  ...
  try {
    ...
    const leadsQuery = `
      SELECT 
        ...
        c.total_interactions as interaction_count,
        c.hot_lead_count,
        c.appointment_count,
        c.phone_request_count,
        c.source_channel as primary_channel,
        c.channels_used,
        c.tags,
        c.created_at,
    ...
    // FIXED: Calculate summary statistics - replaced FILTER with CASE
    const summaryQuery = `
      SELECT 
        COUNT(*) as total,
    ...
    if (leadsResult.rows.length > 0) {
      leadsResult.rows.forEach(lead => {
        const channel = lead.source_channel || lead.primary_channel || 'unknown';
...
export async function getLeadAnalytics(customerId, period = '30d') {
  try {
    ...
    // FIXED: Replaced FILTER syntax with CASE statements
    const analytics = await query(`
      SELECT 
        ...
        SUM(c.total_interactions) as total_interactions,
