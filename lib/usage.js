...
export async function checkCanRespond(clerkUserId, channel) {
  try {
    ...
    if (!limits.channels.includes(mappedChannel)) {
      const channelLabel = mappedChannel.charAt(0).toUpperCase() + mappedChannel.slice(1);
      return {
        allowed: false,
        reason: 'channel_not_included',
        upgradeMessage: `Thanks for reaching out! We'll be in touch with you shortly.`,
        ownerMessage: `${channelLabel} AI is not included in your current plan. Upgrade at bizzybotai.com/pricing to enable it.`,
      };
    }
